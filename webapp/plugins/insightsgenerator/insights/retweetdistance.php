<?php
/*
 Plugin Name: Retweet Distance.
 Description: How many miles did the user help another user's tweet travel.
 When: Weekly, on Sundays
 */
/**
 *
 * ThinkUp/webapp/plugins/insightsgenerator/insights/retweetdistance.php
 *
 ...
 */
class RetweetDistanceInsight extends CriteriaMatchInsightPluginParent implements InsightPlugin {
    ...
    private $seen_retweets = array();

    public function getSlug() {
        return 'retweet_distance';
    }
    ...
    public function postMatchesCriteria(Post $post, Instance $instance) {
        if ($post->in_retweet_of_post_id != NULL && $post->reply_retweet_distance != NULL) {
            $seen = in_array($post->in_retweet_of_post_id, $this->seen_retweets);
            if($seen) {
                return false;
            } else {
                $this->seen_retweets[] = $post->in_retweet_of_post_id;
                return true;
            }
        }
    }
    ...
    // private function checkIfSamePostRetweeted($post) {
    //     // $seen_retweets = array();
    //     $seen = in_array($post->in_retweet_of_post_id, $seen_retweets);
    //     if($seen) {

    //         return false;
    //     } else {
    //         $seen_retweets[] = $post->in_retweet_of_post_id;
    //         return true;
    //     }
    // }
...
$insights_plugin_registrar->registerInsightPlugin('RetweetDistanceInsight');